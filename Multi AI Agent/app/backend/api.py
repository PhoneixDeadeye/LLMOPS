from fastapi import FastAPI, HTTPException
from pydantic import BaseModel
from typing import List
from app.common.custom_exception import CustomException
from app.core.ai_agent import get_response_from_ai_agents
from app.config.settings import settings
from app.common.logger import get_logger
import subprocess
import traceback

logger = get_logger(__name__)

app = FastAPI(title="MULTI AI AGENT")

class RequestState(BaseModel):
    model_name: str
    system_prompt: str
    messages: List[str]
    allow_search: bool  # Add this field

@app.post("/chat")
def chat_endpoint(request: RequestState):
    logger.info(f"Received request for model: {request.model_name}")
     
    if request.model_name not in settings.ALLOWED_MODEL_NAME:
        logger.warning(f"Model not allowed: {request.model_name}")
        raise HTTPException(status_code=400, detail="Model not allowed")
    
    try:
        logger.info(f"Processing request: {request}")
        response = get_response_from_ai_agents(
            request.model_name,
            request.messages,
            request.allow_search,
            request.system_prompt
        )
        logger.info(f"Response: {response}")
        return {"response": response}
    except CustomException as ce:
        logger.error(f"Custom exception in get_response_from_ai_agents: {ce}")
        raise HTTPException(status_code=400, detail=str(ce))
    except Exception as e:
        logger.error(f"Error in get_response_from_ai_agents: {e}")
        logger.error(f"Traceback: {traceback.format_exc()}")
        # In development, return the actual error message
        raise HTTPException(status_code=500, detail=f"Internal Server Error: {str(e)}")

# subprocess.run(["uvicorn", "app.backend.api:app", "--host", "127.0.0.1", "--port", "9999"])